...
namespace PSMS_Sub_DA
{
    public class CUSTOMER_DA
    {
        ...
        public int UpdateCustomerEmail(string username, string email)
        {
            string proc = "SP_Update_AccountEmail";
            SqlParameter[] sqlParams = new SqlParameter[]
            {
                new SqlParameter("@USERNAME",username),
                new SqlParameter("@EMAIL",email)
            };
            return DBHelper.Exec(proc, sqlParams, CommandType.StoredProcedure);
        }

        /// <summary>
        /// Check that the mailbox is unique
        /// </summary>
        /// <param name="email"></param>
        /// <returns>
        /// true: Email is unique
        /// false: Email is not unique
        /// </returns>
